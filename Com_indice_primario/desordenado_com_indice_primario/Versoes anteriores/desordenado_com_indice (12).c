...
//Exclusao - Feito, mas sequencial
//Inserir ordenado - Feito
//Busca binária - Nao


//Reorganização: Feito no arquivo, FALTA NO INDICE! Ps, atualizar deslocamento
...
int main() //passar num_reg por args[] na main
{
      int   comp_reg;
      int comp_reg_indice;
...
/* Simulando a leitura do arquivo*/
    ...
    //Limpa arquivo de indice
    //remove("indice_desordenado.bin");

   // FILE *fp_indice = fopen("indice_desordenado.bin","wb");
    //fclose(fp_indice);
     //int desl = sizeof(int);
  ...
  for (r=0; r < num_reg; r++) {
    ...
    pos3 += sizeof(float);
        ...
        pos3=0;
           ...
           printf(" aca->%d<-aca\n",num_reg2 );
            num_reg2--;
             ...
             // fp_indice = fopen("indice_desordenado.bin","rb+");
              //fwrite(&num_reg2,sizeof(int),1,fp_indice);
              //fclose(fp_indice);
     ...
     //insere_ordenado_indice(b.RA,20);
    // if (b.RA>-1) desl=desl+comp_reg+sizeof(int);
