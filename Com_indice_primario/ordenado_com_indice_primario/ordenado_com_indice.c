...
//Insere ancora ordenada no arquivo de indices
void insere_ordenado_indice(int RA, int deslocamento);
//Imprime arquivo de indices
void imprime_indice();
//Cria arquivo de dados e de indices
void cria_arquivos(Aluno *a, ancora *c);
//Insere registro ordenado no arquivo
void insereOrdenado(Aluno a);
//Busca posicao para inserção ordenada
int buscaPosicao(int RA);
//Reorganiza arquivo de dados e recria arquivo de indices
void reorganizacao_dados();
//Cria arquivo de indices
void cria_indice();
//Imprime todos dados do arquivo de dados
void imprime_dados();
//Função que recupera um registro dado um deslocamento no arquivo principal
Aluno* recupera_registro(int deslocamento);
//Funcao que busca um RA no indice
ancora* busca_indice(int chave);
//Remove logicamente registro do arquivo de dados e do arquivo de indices(marca com -1)
...
int main()
{
  ...
  for (r=0; r < TAM_MAX; r++) {
   ...
   cria_indice(); //Cria arquivo de indices
         ...
         while((fgets(linha,sizeof(linha), arq))!=NULL )
          {
            if (linha[0] == 'a'){
                ...
                comando= (char*)strtok(linha, " ");
                registro= (char*)strtok(NULL, " ");
                 ancora *anc =  busca_indice(atoi(registro));
                 recupera_registro(anc->deslocamento);
...
void insereOrdenado(Aluno a)
{
    ...
    pos1=0;
            ...
            fseek(fp,0,SEEK_SET);
            fwrite(&num_reg,sizeof(char),sizeof(int),fp);
            fclose(fp);
  ...
  for (r=0; r < num_reg; r++) {
    ...
    pos3 += sizeof(float);
        ...
        pos3=0;
            ...
            int num_reg2;
            //atualiza num_reg e grava no arquivo /
            pos3=0;
