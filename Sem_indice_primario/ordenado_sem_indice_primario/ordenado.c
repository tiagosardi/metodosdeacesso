...
void imprimeBusca(int deslocamento, int chave)
{
         ...
         while((fgets(linha,sizeof(linha), arq))!=NULL )
          {
            if (linha[0] == 'a'){
                ...
                comando= (char*)strtok(linha, " ");
                registro= (char*)strtok(NULL, " ");
                num =buscaPosicao(atoi(registro));
                imprimeBusca(num,atoi(registro));
